...
describe('PATCH /products/:sku', () => {
    it('should decrement a product inventory count by 1', async () => {
        let testSKU = testProducts[0].sku;
        let response = await request(app).patch(`/products/${testSKU}`).expect(200);
    ...
    it('should return 404 if product not found', async () => {
        let fakeSKU = 5;
        let response = await request(app).patch(`/products/${fakeSKU}`).expect(404);
    });

    it('should return 400 if SKU is NaN', async () => {
        let fakeSKU = 'isThisASKU';
        let response = await request(app).patch(`/products/${fakeSKU}`).expect(400);
    });

    it('should return 404 if product has no inventory', async () => {
        let testSKU = testProducts[2].sku;
        let response = await request(app).patch(`/products/${testSKU}`).expect(404);
    });
})